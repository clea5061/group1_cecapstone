...
def main():
    ...
    train_x, train_y, test_x, test_y = load_data(path, 0.2)
    ...
    for i in range(num_epochs):
        ...
        for j in range(0, batches_per_epoch):
            ...
            for k in range(len(batch[0])):
                batch_[0].append(batch[0][k].astype(dtype=np.float32)/255.0)
                batch_[1].append(np.array([batch[1][k][0]], dtype=np.float32))
            batch = batch_
        ...
        if i % 5 == 0:
            ...
            print("epoch:" + str(i) + " test accuracy %g" % test_accuracy)
